...
// ── Data per tab ─────────────────────────────────────────────────────────────
const DATA: Record<TabOption, { label: string; value: number }[]> = {
    Diario: [
        { label: 'L',  value: 320 },
        { label: 'M',  value: 410 },
        { label: 'X',  value: 380 },
    ...
    Mensual: [
        ...
        { label: 'Abr', value: 10200 },
        { label: 'May', value: 8900 },
        { label: 'Jun', value: 11400 },
    ],
};

const THRESHOLD: Record<TabOption, number> = {
    Diario: 450,
    Semanal: 2600,
    Mensual: 9500,
};

const OVER_COUNT: Record<TabOption, number> = {
    Diario: 2,
    Semanal: 1,
    Mensual: 3,
};
...
export default function ConsumptionChart({ activeTab }: ConsumptionChartProps) {
    const [animKey, setAnimKey] = useState(0);
    const [chartVisible, setChartVisible] = useState(true);

    // Re-animate bars whenever tab changes
    useEffect(() => {
    ...
    const overCount = OVER_COUNT[activeTab];
            ...
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2 bg-amber-50 rounded-xl px-3 py-1.5">
                    ...
                    <p className="text-sm text-slate-700">
                        <span className="font-bold text-amber-500">{overCount} {activeTab === 'Diario' ? 'dias' : activeTab === 'Semanal' ? 'semanas' : 'meses'}</span>
          ...
          EN VIVO
            ...
            >
                <ResponsiveContainer width="100%" height={220}>
                    <BarChart key={animKey} data={data} margin={{ top: 8, right: 8, left: -20, bottom: 0 }} barSize={28}>
                        ...
                        <ReferenceLine
                            ...
                            strokeDasharray="5 4"
                            strokeWidth={1.5}
                            label={{ value: 'Umbral', position: 'insideTopLeft', fontSize: 10, fill: '#f59e0b', dy: -6 }}
                        />
                        <Bar dataKey="value" radius={[6, 6, 0, 0]} animationDuration={700} animationEasing="ease-out">
                            {data.map((entry, i) => (
                                <Cell
                                    key={i}
